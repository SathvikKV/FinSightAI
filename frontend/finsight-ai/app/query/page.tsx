...
import { queryFilings, generateRagAnswer } from "@/lib/api"; // ✅ Use API utility
...
export default function QueryAssistant() {
  ...
  const topK = Number.parseInt(searchParams.get("topK") || "5") || 5;
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      ...
      setError(null);
      try {
        const data = await queryFilings(query, topK, formType);
  ...
  const handleGetGptAnswer = async () => {
    ...
    setError(null);
    try {
      const data = await generateRagAnswer(query!, topK, formType);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="max-w-3xl mx-auto mb-8 premium-border p-6 rounded-xl backdrop-blur-sm">
        <SearchForm
          initialQuery={query || ""}
          initialFormType={formType}
          initialTopK={topK.toString()}
        />
      </div>
      ...
      {!query && (
        <div className="mt-12">
          ...
          <FloatingQueries className="mt-4" />
        </div>
      )}
